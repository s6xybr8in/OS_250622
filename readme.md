...
### [2026-04-26] 커널 아키텍처 계층화 및 설계 최적화
**1. 계층화(Layering) 원칙의 이해**
- **관심사 분리**: CPU 종속적 코드(Arch), 하드웨어 제어(Drivers), 커널 핵심 로직(Core)을 분리하여 복잡도를 관리하는 방법 학습.
- **추상화 및 이식성**: HAL(Hardware Abstraction Layer)의 개념을 적용하여 향후 다른 CPU 아키텍처 지원 가능성을 고려한 설계의 중요성 파악.
- **유지보수성**: 소스 코드의 물리적 위치(디렉토리 구조)를 논리적 역할에 맞게 재배치함으로써 디버깅 및 확장 시의 이점을 이해함.

**2. 파일 구조 리팩토링 설계**
- `src/kernel/arch/i386/`: GDT, IDT, PIC 등 CPU 관련 로직 격리.
- `src/kernel/drivers/`: 시리얼, 타이머 등 장치별 드라이버 계층화.
- `src/include/`: 헤더 파일의 구조적 배치 및 전역 인터페이스 정의.

**3. 향후 과제 (To-do)**
- [ ] 현재 평면적인 `src/` 구조를 계층형 구조로 리팩토링.
- [ ] 변경된 구조에 맞게 `makefile`의 빌드 규칙 수정.
- [ ] `asmb.h` 등의 인라인 어셈블리를 아키텍처 폴더로 이동 및 정리.
---